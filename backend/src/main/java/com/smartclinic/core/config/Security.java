...
public class Security {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        // Vô hiệu hóa CSRF (bảo vệ chống giả mạo request liên trang) để đơn giản hóa
        // khi phát triển
        http.csrf(csrf -> csrf.disable())
                // Kích hoạt CORS với cấu hình mặc định (sẽ dùng bean corsConfigurationSource
                // bên dưới)
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        // (Cho phép truy cập vào các API bắt đầu bằng "/api/auth/" như đăng nhập, đăng
                        // ký mà không cần xác thực)
                        .requestMatchers(
                                "/api/v1/auth/**",
                                "/api/auth/**",
                                "/v3/api-docs/**",
